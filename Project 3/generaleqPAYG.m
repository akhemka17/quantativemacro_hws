...
for it=1:maxit,
    [fval,ass,Y,vfun,gridx,wage,PhiAss]= func_olg(ret);
    if (abs(fval)<tol),
        disp('convergence');
        break;
    else
...
x=squeeze(gridx(1,:,:));
   ...
   save vT_dec.txt v0 -ascii -double %if replrate == 0.0
  save xT_dec.txt x -ascii -double
%   save vT1_dec.txt v0 -ascii -double %if replrate == 0.6
 %  save xT1_dec.txt x -ascii -double    
...
global R replrate betta tetta nj jr nx ny pi gridy netw pens sr epsi curv pini frac pop totpop grdfac delta alpha L
...
% retired share of the population
R = sum(pop(jr+1:nj)); 
...
global L  delta gridx replrate R
...
[gridx,gridsav,gridass,cfun,vfun] = func_hh(wage,ret,tau);
...
global betta tetta nj nx ny pi gridy pens sr epsi curv grdfac replrate L R
